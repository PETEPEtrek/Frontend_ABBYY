    ...
    return (
            <div className="flex-1 px-7 sm:px-5">
                ...
                <div className="mb-10 sm:mb-0 mt-10 grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                    {filteredArticles.map((character, index) => (
                        <a href={`/characters/${character.ID}`} key={index}
                           className="relative group bg-gray-900 py-10 sm:py-20 px-4 flex flex-col space-y-2 items-center cursor-pointer rounded-md hover:bg-gray-900/80 hover:smooth-hover">
                            <img className="w-20 h-20 object-cover object-center object-scale-down rounded-full"
                                 src={character.Image}/>
                            <h4 className="text-white text-2xl font-bold capitalize text-center">{character.Name}</h4>
                            <p className="text-white/50">{character.VoteNumber}</p>
                        </a>
                    ))}
                </div>
            </div>
...
export default CharacterMainContent;